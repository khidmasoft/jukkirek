...
import com.khidmasoft.jukkirek.config.KafkaProperties;
...
class JukkirekKafkaResourceIT {
    ...
    void setup() {
        ...
        JukkirekKafkaResource kafkaResource = new JukkirekKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jukkirek-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jukkirek-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
